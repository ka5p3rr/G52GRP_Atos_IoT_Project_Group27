...
public class NavigationActivity extends AppCompatActivity
  ...
  /**
   * Overrides the back button functionality. Back button only closes the {@link DrawerLayout} when
   * open. Otherwise button press is ignored.
   */
  @Override
  ...
  /**
   * Inner class that extends {@link Thread} and runs the TCP IP client. On change updates the UI
   * using {@link Handler}.
   */
      ...
      try {
        ...
        if (data.contains(getString(R.string.default_data))) {
          ...
          if (values.length >= 2) {
            ...
            if (i > previousValue) createNotification("Tank capacity " + i + "%");
            if (i < previousValue) cancelNotification();
            previousValue = i;
